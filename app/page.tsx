...
const sellCategories = categories.filter((item) => item !== "All listings");
...
function SellModal({ apiUrl, onCreated, onClose }: { apiUrl: string; onCreated: () => void; onClose: () => void }) {
  ...
  const formik = useFormik({
    initialValues: { title: "", category: sellCategories[0], subcategory: "", brand: "", model: "", yearPurchased: "", price: "", originalPrice: "", desc: "" },
    validate: (values) => {
    ...
    onSubmit: async (values, helpers) => {
      ...
      try {
        ...
        const payload = {
          ...
          ...(values.subcategory.trim() && { subcategory: values.subcategory.trim() }),
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="sell-modal"><button className="modal-close" onClick={onClose} aria-label="Close"><X size={20} /></button><div className="sell-intro"><p className="eyebrow">Sell with Circle</p><h2>List your item.</h2><p>Add a few details and photos to get your item in front of buyers.</p></div><div className="sell-form"><form onSubmit={submit}>
    ...
    <div className="sell-row">
      <label>Category<select name="category" value={formik.values.category} onChange={formik.handleChange}>{sellCategories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label>Subcategory<input name="subcategory" value={formik.values.subcategory} onChange={formik.handleChange} placeholder="Optional" /></label>
    </div>
    <div className="sell-row">
